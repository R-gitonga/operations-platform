...
fn validate_quantities(
    ...
    if qty_raised < 0 {
        return Err(AppError::BadRequest(
            "qty_raised cannot be negative".into(),
        ));
    }
...
pub async fn update(
    ...
    item.balance =
        item.qty_raised - item.qty_received;
...
pub async fn receive(
    ...
    let new_received =
        item.qty_received + payload.quantity;

    if new_received > item.qty_raised {
        return Err(AppError::BadRequest(
            "Received quantity exceeds quantity raised."
                .into(),
        ));
    }
...
pub async fn delete(
    ...
    let deleted =
        line_item::delete(pool, line_item_id).await?;
